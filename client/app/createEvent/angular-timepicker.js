    ...
    angular.module('goApp')
        ...
        .directive('dnTimepickerPopup', function () {
            return {
                ...
                replace: true,
                transclude: false,
                template: '<ul class="dn-timepicker-popup dropdown-menu" style="height: auto; max-height: 185px; overflow-x:hidden;" ng-style="{display: timepicker.isOpen && \'block\' || \'none\', top: position.top+\'px\', left: position.left+\'px\'}"><li ng-repeat="time in timepicker.optionList()" ng-class="{active: isActive($index) }" ng-mouseenter="setActive($index)"><a ng-click="select(time)">{{time | date:timepicker.timeFormat}}</a></li></ul>',
